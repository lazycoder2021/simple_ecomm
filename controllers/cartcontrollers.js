...
module.exports.get_cart_items = async (req, res) => {
    ...
    try {
        ...
        if (cart) { //even this works perfectly 
...
module.exports.add_cart_item = async (req, res) => {
    ...
    try {
        ...
        if (cart) {
            ...
            if (itemIndex > -1) {
                let productItem = cart.items[itemIndex]; // this will take some getting used to....
...
module.exports.update_cart_item = async (req, res) => {
    ...
    try {
        ...
        if (!cart)
            ...
            cart.bill = cart.items.reduce((sum, item) => sum + item.price * item.quantity, 0); // not sure y this was used and y the above code is not working
            cart = await cart.save();
            return res.status(201).send(cart);
        }
    }
